import { withProfile } from './gemini.js';
...
export async function generateBuddies(topic, profile) {
  if (!topic) return { buddies: [], error: 'お題を入力してください。' };

  const sys = withProfile(BUDDY_GEN_SYS, profile);
  let lastErr = '';
  for (let i = 0; i < 3; i++) {
    try {
      const res = await callLLM(sys, `お題: ${topic}`, true, true);
      if (!res) { lastErr = '応答が空でした'; continue; }
      if (/high demand|overloaded|unavailable|503|APIエラー|接続エラー|rate limit|429/i.test(res)) {
        lastErr = res;
...
export async function chatWithBuddy(buddy, history, userMsg, userName) {
  ...
  for (let i = 0; i < 3; i++) {
    const res = await callLLM(sys, prompt, false, true);
    if (res && !/high demand|overloaded|unavailable|503|rate limit|429/i.test(res)) return res;
    await new Promise(r => setTimeout(r, 1500 * (i + 1)));
  }
