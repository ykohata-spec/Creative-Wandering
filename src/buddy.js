import { callGemini, withProfile } from './gemini.js';
...
export async function generateBuddies(apiKey, topic, profile) {
  ...
  for (let i = 0; i < 3; i++) {
    try {
      const res = await callGemini(apiKey, sys, `お題: ${topic}`, true, true);
